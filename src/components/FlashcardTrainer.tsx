  ...
  return (
    <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_320px] xl:grid-cols-[minmax(0,1fr)_360px]">
      <section className="overflow-hidden rounded-lg border border-stone-200 bg-white shadow-sm">
        <div className="flex items-center justify-between gap-3 border-b border-stone-200 px-4 py-3 sm:px-6">
          <div className="min-w-0">
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-teal-700">Флэшкарта</p>
            <h1 className="mt-1 truncate text-lg font-semibold text-stone-950 sm:text-2xl">Выбери ударную гласную</h1>
          </div>
          <div className="inline-flex shrink-0 items-center gap-2 rounded-md bg-stone-100 px-3 py-2 text-sm text-stone-700">
            ...
            <span className="hidden sm:inline">{stressWords.length} слов всего</span>
        ...
        <div className="px-3 py-4 sm:px-6 sm:py-8">
          <div className="mb-4 grid grid-cols-4 gap-1.5 sm:mb-5 sm:gap-2">
            {trainingModes.map((item) => (
              <button
                ...
                className={[
                  "rounded-md border px-2 py-2 text-center transition sm:px-3 sm:py-3 sm:text-left",
              ...
              >
                ...
                <span className="mt-1 hidden text-xs leading-4 text-stone-500 sm:block">{item.description}</span>
              </button>
            ))}
          </div>
          ...
          <div className="mb-4 flex flex-wrap items-center gap-1.5 sm:mb-5 sm:gap-2">
            ...
            <span className="rounded-md bg-stone-100 px-2.5 py-1 text-xs text-stone-700 sm:px-3 sm:text-sm">
              точность: {formatAccuracy(accuracy(currentStat))}
            </span>
            <span className="rounded-md bg-teal-100 px-2.5 py-1 text-xs font-medium text-teal-900 sm:px-3 sm:text-sm">
              {currentProfile.reason}, мастерство {currentProfile.mastery}%
            </span>
          </div>

          <div className="rounded-lg border border-stone-200 bg-stone-50 px-2 py-7 sm:px-6 sm:py-8">
            <div className="flex min-h-28 flex-wrap items-end justify-center gap-1 sm:min-h-24 sm:gap-2">
              {choices.map((choice) => {
                ...
                return (
                  <button
                    ...
                    className={[
                      "relative flex h-14 min-w-8 items-center justify-center rounded-md border px-1.5 text-[clamp(1.65rem,8vw,2.5rem)] font-semibold transition sm:h-20 sm:min-w-12 sm:px-2 sm:text-5xl",
                      choice.isVowel
                        ? "touch-manipulation border-stone-300 bg-white text-stone-950 active:scale-95 sm:hover:-translate-y-0.5 sm:hover:border-teal-500 sm:hover:bg-teal-50"
          ...
          <div className="mt-5 flex min-h-16 flex-col items-stretch justify-between gap-3 sm:flex-row sm:items-center">
            {lastResult === null ? (
              <p className="flex items-center gap-2 text-sm text-stone-600">
                <Keyboard className="h-4 w-4" />
                <span className="sm:hidden">Нажимай на гласную. Дальше пойдет само.</span>
                <span className="hidden sm:inline">Нажимай мышкой или цифрами 1-{vowelChoices.length}. После ответа карточка сменится сама.</span>
              </p>
            ) : (
              <div
                className={[
            ...
            <button
              type="button"
              onClick={() => nextWord()}
              className="inline-flex h-12 items-center justify-center gap-2 rounded-md bg-stone-950 px-4 text-sm font-semibold text-white transition hover:bg-stone-800 disabled:cursor-not-allowed disabled:bg-stone-300 sm:h-11"
              disabled={!answered}
            >
      ...
      <aside className="space-y-3 lg:space-y-5">
        <div className="rounded-lg border border-stone-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-3 grid grid-cols-4 gap-1.5 lg:grid-cols-2 lg:gap-2">
        ...
        <div className="hidden rounded-lg border border-stone-200 bg-white p-4 shadow-sm sm:block">
          <h2 className="flex items-center gap-2 text-sm font-semibold uppercase tracking-[0.14em] text-stone-500">
